...
function setupAgent(){
  agent=createAgent({baseUrl:process.env.ELP_DEVICE_SERVER_URL||'https://elpgpt.com',statePath:path.join(app.getPath('userData'),'companion.json')});
  agent.start(Number(process.env.ELP_DEVICE_POLL_MS||15000));
}
function setupTray(){
  tray=new Tray(nativeImage.createEmpty());
  tray.setToolTip('ELP Companion');
  tray.setContextMenu(Menu.buildFromTemplate([{label:'Open ELP Companion',click:()=>win.show()},{label:'Open ELP',click:()=>void shell.openExternal('https://elpgpt.com')},{type:'separator'},{label:'Quit',click:()=>{app.isQuitting=true;app.quit();}}]));
  tray.on('click',()=>win.show());
}
app.whenReady().then(()=>{setupAgent();createWindow();setupTray();win.show();});
app.on('before-quit',()=>{app.isQuitting=true;agent?.stop();});
app.on('window-all-closed',()=>{});

ipcMain.handle('companion:status',()=>agent.status());
