...
export function isPublicApiRoute(pathname:string){if(PUBLIC_EXACT.has(pathname))return true;return PUBLIC_PREFIXES.some((prefix)=>pathname.startsWith(prefix));}
export function requiredApiCapability(pathname:string,method:string):AuthorityCapability{
 const verb=method.toUpperCase();if(verb==='GET'||verb==='HEAD'||verb==='OPTIONS')return'read_context';
 if(pathname.startsWith('/api/authority-control')||pathname.startsWith('/api/security-operations')||pathname.startsWith('/api/security-automation')||pathname.startsWith('/api/standing-authority'))return'manage_authority';
