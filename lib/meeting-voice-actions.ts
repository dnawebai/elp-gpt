...
export async function resolveMeetingReference(profileId: string, reference: string, timezone = 'America/Toronto') {
  ...
  if (dateNeedle) {
    ...
    if (sameDay.length > 1) {
      const semanticNeedle = needle.replace(/yesterday|today|meeting|the|my/g, '').trim();
      const semantic = semanticNeedle ? sameDay.filter((meeting) => searchableMeetingText(meeting).includes(semanticNeedle)) : [];
