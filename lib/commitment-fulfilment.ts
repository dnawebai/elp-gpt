...
function buildCandidates(ledger: Awaited<ReturnType<typeof getExecutiveLedger>>, board: Awaited<ReturnType<typeof getTaskBoard>>) {
  ...
  for (const item of ledger.items) {
    ...
    candidates.push({ targetType: 'commitment', targetId: item.id, title: item.title, objective: item.content, priority: candidatePriority(item), dueDate: item.dueDate, commitment: item });
  }
...
function shouldSkip(latest: CommitmentFulfilmentRecord | undefined, force: boolean) {
...
async function ensureDecisionTask(profileId: string, candidate: Candidate, result: { summary: string; pendingAction?: OperatorPendingAction; question?: string }, board: Awaited<ReturnType<typeof getTaskBoard>>) {
  ...
  const existing = [...board.queues.now, ...board.queues.decisions, ...board.queues.working, ...board.queues.delegated].find((task) => task.sessionId === sessionId && task.status !== 'completed' && task.status !== 'cancelled');
  const objective = result.pendingAction ? `Approval needed to advance commitment: ${candidate.title}. ${result.summary}` : `Input needed to advance commitment: ${candidate.title}. ${result.question || result.summary}`;
  if (existing) {
    await updateTask(profileId, existing.id, { queue: 'decisions', owner: 'user', approval: result.pendingAction ? 'required' : 'none', status: 'blocked', summary: result.summary, toolSlug: result.pendingAction?.toolSlug || null, risk: result.pendingAction?.risk || null, note: result.question || null });
    return existing.id;
  }
  return createTask(profileId, { objective, queue: 'decisions', owner: 'user', priority: candidate.priority >= 100 ? 'critical' : candidate.priority >= 70 ? 'high' : 'normal', approval: result.pendingAction ? 'required' : 'none', source: 'commitment-fulfilment', sessionId, summary: result.summary, toolSlug: result.pendingAction?.toolSlug, risk: result.pendingAction?.risk });
}
...
async function applyResult(profileId: string, candidate: Candidate, result: Awaited<ReturnType<typeof runOperatorMission>>, board: Awaited<ReturnType<typeof getTaskBoard>>) {
  if (candidate.targetType === 'task' && candidate.task) {
    if (result.status === 'completed') await updateTask(profileId, candidate.task.id, { queue: 'done', owner: 'ai', approval: 'none', status: 'completed', summary: result.summary, evidence: result.summary, note: 'Completed by autonomous read-only/reversible fulfilment cycle.' });
    else if (result.status === 'approval_required') await updateTask(profileId, candidate.task.id, { queue: 'decisions', owner: 'user', approval: 'required', status: 'blocked', summary: result.summary, toolSlug: result.pendingAction?.toolSlug, risk: result.pendingAction?.risk, note: 'External write requires explicit approval.' });
    else if (result.status === 'needs_input') await updateTask(profileId, candidate.task.id, { queue: 'decisions', owner: 'user', approval: 'none', status: 'blocked', summary: result.summary, note: result.question || 'User input is required.' });
    else await updateTask(profileId, candidate.task.id, { summary: result.summary, note: 'Autonomous fulfilment is currently blocked; no external write was executed.' });
    return;
  }
...
export async function runCommitmentFulfilmentCycle(args: { profileId: string; sessionPrefix?: string; limit?: number; force?: boolean }) {
  const limit = Math.max(1, Math.min(8, Math.round(args.limit || 4)));
  const [ledger, board, snapshot] = await Promise.all([getExecutiveLedger(args.profileId), getTaskBoard(args.profileId), getCommitmentFulfilmentSnapshot(args.profileId)]);
  ...
  for (const candidate of candidates) {
    ...
    if (shouldSkip(prior, Boolean(args.force))) continue;
    reviewed += 1;
    try {
      const mission = await runOperatorMission({ objective: missionObjective(candidate), profileId: args.profileId, sessionId: `${args.sessionPrefix || 'commitment-fulfilment'}-${reviewed}`.slice(0, 120), state: null });
      await applyResult(args.profileId, candidate, mission, board);
      const nextReviewAt = mission.status === 'approval_required' || mission.status === 'needs_input' ? undefined : new Date(Date.now() + RECENT_REVIEW_MS).toISOString();
      const id = await persistRecord(args.profileId, { targetType: candidate.targetType, targetId: candidate.targetId, title: candidate.title, status: mission.status, summary: mission.summary, ...(mission.pendingAction ? { pendingAction: mission.pendingAction } : {}), ...(mission.question ? { question: mission.question } : {}), ...(nextReviewAt ? { nextReviewAt } : {}) });
      results.push({ id: id || `${candidate.targetType}-${candidate.targetId}`, targetType: candidate.targetType, targetId: candidate.targetId, title: candidate.title, status: mission.status, summary: mission.summary, createdAt: new Date().toISOString(), ...(mission.pendingAction ? { pendingAction: mission.pendingAction } : {}), ...(mission.question ? { question: mission.question } : {}), ...(nextReviewAt ? { nextReviewAt } : {}) });
    } catch (error) {
  ...
  return { ok: true, reviewed, candidateCount: candidates.length, results, approvalRequired: results.filter((item) => item.status === 'approval_required').length, completed: results.filter((item) => item.status === 'completed').length, needsInput: results.filter((item) => item.status === 'needs_input').length, blocked: results.filter((item) => item.status === 'blocked').length };
}
...
export async function recordFulfilmentExecuted(profileId: string, recordId: string, evidence?: string) {
  ...
  const metadata: Record<string, unknown> = { ...found.message.metadata, fulfilmentStatus: 'completed', updatedAt: now, nextReviewAt: new Date(Date.now() + RECENT_REVIEW_MS).toISOString(), ...(evidence ? { evidence: clip(evidence, 3000) } : {}) };
...
export function commitmentFulfilmentToPrompt(snapshot: CommitmentFulfilmentSnapshot) {
  ...
  return [`Autonomous fulfilment totals: ${snapshot.stats.approvalRequired} awaiting approval; ${snapshot.stats.needsInput} need input; ${snapshot.stats.blocked} blocked.`, live.length ? `Live fulfilment queue:\n${live.join('\n')}` : ''].filter(Boolean).join('\n\n');
}
