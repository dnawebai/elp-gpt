...
export type ZeroTrustAuthorityContext = {
  profileId: string;
  principal: Principal;
  delegated: boolean;
...
export async function resolveZeroTrustAuthority(request: Request): Promise<ZeroTrustAuthorityContext | null> {
  ...
  if (!claims || claims.profileId !== profile.profileId) return null;
  ...
  return { profileId: profile.profileId, principal, delegated: true, session };
}
...
export function requiresDelegatedStepUp(context: ZeroTrustAuthorityContext, risk: 'read' | 'write' | 'high') {
  return context.delegated && risk === 'high';
}
