...
export default function Home() {
  ...
  const sendTypedMessage = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok || !data?.message) throw new Error(data?.error || 'LUKE did not return a response.');
      ...
      setVoiceError(error instanceof Error ? error.message : 'Unable to send message to LUKE.');
    } finally {
  ...
  const stateLabel = useMemo(() => {
    ...
    if (inputMode === 'text') return 'TYPE TO LUKE';
  ...
  function renderSection() {
    ...
    if (activeSection === 'skills') return <section className="detail-card"><div className="detail-heading"><div><span>SKILLS</span><h3>Active capabilities</h3></div></div><div className="capability-grid"><span>Realtime Deepgram voice</span><span>Typed LUKE chat</span><span>Composio tool discovery</span><span>Permission-gated actions</span><span>Hermes / Together routing</span><span>Honcho memory continuity</span><span>System diagnostics</span></div></section>;
  ...
  return <main className={`luke-shell ${motionMode}`}>
    ...
    <section className="voice-stage" aria-label="LUKE interaction interface">
      <button className={`hud ${voice.isActive ? 'is-active' : ''} ${voice.isSpeaking ? 'is-speaking' : ''} ${voice.isListening ? 'is-listening' : ''} ${voice.isThinking || isSendingText ? 'is-thinking' : ''}`} onClick={toggleVoice} aria-label={inputMode === 'text' ? 'LUKE visual interface' : voice.isActive ? 'Stop LUKE voice session' : 'Activate LUKE voice session'}>
        ...
        <span className="hud-center"><span className="voice-wave" aria-hidden="true"><i /><i /><i /><i /><i /><i /><i /></span><strong>LUKE</strong><span className="voice-state">{inputMode === 'text' ? <Keyboard size={14} /> : <Mic size={14} />}{stateLabel}</span></span>
      </button>
      <div className={`active-state ${(voice.isActive || inputMode === 'text') ? 'online' : ''}`}><i /><span>{activeLabel}</span></div>
      {voiceError && <p className="voice-error">{voiceError}</p>}
    </section>

    <section className={`input-dock mode-${inputMode}`} aria-label="LUKE input controls">
      {inputMode === 'text' && latestTextTurn && <p className={`text-turn ${latestTextTurn.role}`}>{latestTextTurn.content}</p>}
      <div className="input-mode-switch" role="tablist" aria-label="Choose LUKE input mode">
      ...
      {inputMode === 'text' ? <form className="text-composer" onSubmit={(event) => { event.preventDefault(); void sendTypedMessage(); }}>
        <input value={typedMessage} onChange={(event) => setTypedMessage(event.target.value)} placeholder="Write to LUKE…" aria-label="Write a message to LUKE" autoComplete="off" disabled={isSendingText} />
