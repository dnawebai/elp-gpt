...
import { useElpVoice, type ElpVoiceCommand } from '@/lib/useElpVoice';
...
export default function Home() {
  ...
  const [textMessages, setTextMessages] = useState<VoiceMessage[]>([]);
  ...
  const handleVoiceCommand = useCallback(async ({ name, input }: ElpVoiceCommand) => {
    ...
    return { ok: false, error: `Unsupported ELP command: ${name}` };
  ...
  const switchInputMode = useCallback((mode: InputMode) => {
    ...
    if (mode === 'text' && voice.isActive) voice.stop();
    setVoiceError(null);
    setInputMode(mode);
  }, [inputMode, voice]);
  ...
  const sendTypedMessage = useCallback(async () => {
    ...
    if (!content || isSendingText) return;

    const userMessage: VoiceMessage = { role: 'user', content };
    const nextMessages = [...textMessages, userMessage].slice(-18);
    setTextMessages(nextMessages);
    setLatestVoice(userMessage);
    setTypedMessage('');
    setVoiceError(null);
    ...
    try {
      ...
      if (!response.ok || !data?.message) throw new Error(data?.error || 'ELP did not return a response.');

      const assistantMessage: VoiceMessage = { role: 'assistant', content: data.message };
      setTextMessages((current) => [...current, assistantMessage].slice(-18));
      setLatestVoice(assistantMessage);
      if (activeSection === 'memory') void loadMemory();
    } catch (error) {
      setVoiceError(error instanceof Error ? error.message : 'Unable to send message to ELP.');
    } finally {
      setIsSendingText(false);
    }
  }, [activeSection, isSendingText, loadMemory, sessionId, textMessages, typedMessage]);

  const stateLabel = useMemo(() => {
    if (inputMode === 'text' && isSendingText) return 'THINKING';
    if (inputMode === 'text') return 'TYPE TO ELP';
  ...
  }, [inputMode, isSendingText, voice.isListening, voice.isSpeaking, voice.isThinking, voice.state]);

  const activeLabel = inputMode === 'text'
    ? (isSendingText ? 'PROCESSING' : 'TEXT MODE')
    : voice.isActive ? 'ACTIVE' : voice.state === 'error' ? 'CHECK SYSTEM' : 'STANDBY';

  const motionMode = inputMode === 'text'
    ? (isSendingText ? 'mode-thinking' : 'mode-idle')
    : voice.isSpeaking ? 'mode-speaking'
      : voice.isThinking ? 'mode-thinking'
        : voice.isListening ? 'mode-listening'
          : 'mode-idle';
  ...
  function renderSection() {
    if (activeSection === 'memory') {
      return <section className="detail-card"><div className="detail-heading"><div><span>MEMORY</span><h3>What ELP knows</h3></div><button className="mini-button" onClick={() => void loadMemory()} aria-label="Refresh memory"><RefreshCw size={15} /></button></div>{loadingMemory && <p className="section-note">Reading current memory…</p>}{!loadingMemory && memory && !memory.configured && <p className="section-note">Honcho is not configured on this deployment.</p>}{!loadingMemory && memory?.configured && !memory.available && <p className="section-note">Memory is connected. Keep talking with ELP and stable context will appear here.</p>}{memory?.facts?.length ? <div className="fact-list">{memory.facts.map((fact, index) => <p key={`${index}-${fact}`}>{fact}</p>)}</div> : null}{memory?.summary ? <div className="memory-block"><span>SESSION CONTEXT</span><p>{memory.summary}</p></div> : null}{memory?.representation ? <div className="memory-block"><span>PROFILE MODEL</span><p>{memory.representation}</p></div> : null}</section>;
    }
    if (activeSection === 'signals') return <section className="detail-card"><div className="detail-heading"><div><span>SIGNALS</span><h3>Attention radar</h3></div></div><p className="section-note">The signal surface is ready for connected monitors. ELP only reports verified connected signals.</p></section>;
    if (activeSection === 'skills') return <section className="detail-card"><div className="detail-heading"><div><span>SKILLS</span><h3>Active capabilities</h3></div></div><div className="capability-grid"><span>Realtime Deepgram voice</span><span>Typed ELP chat</span><span>Composio tool discovery</span><span>Permission-gated actions</span><span>Hermes / Together routing</span><span>Honcho memory continuity</span><span>System diagnostics</span></div></section>;
  ...
  const latestTextTurn = inputMode === 'text' ? textMessages[textMessages.length - 1] : null;

  return <main className={`elp-shell ${motionMode}`}>
    ...
    <section className="voice-stage" aria-label="ELP interaction interface">
      <button className={`hud ${voice.isActive ? 'is-active' : ''} ${voice.isSpeaking ? 'is-speaking' : ''} ${voice.isListening ? 'is-listening' : ''} ${voice.isThinking || isSendingText ? 'is-thinking' : ''}`} onClick={toggleVoice} aria-label={inputMode === 'text' ? 'ELP visual interface' : voice.isActive ? 'Stop ELP voice session' : 'Activate ELP voice session'}>
        <span className="hud-halo" /><span className="hud-ring ring-outer" /><span className="hud-ring ring-arc-a" /><span className="hud-ring ring-arc-b" /><span className="hud-ring ring-ticks" /><span className="hud-ring ring-inner" />
        <span className="hud-center"><span className="voice-wave" aria-hidden="true"><i /><i /><i /><i /><i /><i /><i /></span><strong>ELP</strong><span className="voice-state">{inputMode === 'text' ? <Keyboard size={14} /> : <Mic size={14} />}{stateLabel}</span></span>
      </button>
      <div className={`active-state ${(voice.isActive || inputMode === 'text') ? 'online' : ''}`}><i /><span>{activeLabel}</span></div>
      {voiceError && <p className="voice-error">{voiceError}</p>}
    </section>

    <section className={`input-dock mode-${inputMode}`} aria-label="ELP input controls">
      {inputMode === 'text' && latestTextTurn && <p className={`text-turn ${latestTextTurn.role}`}>{latestTextTurn.content}</p>}
      <div className="input-mode-switch" role="tablist" aria-label="Choose ELP input mode">
        <button type="button" role="tab" aria-selected={inputMode === 'voice'} className={inputMode === 'voice' ? 'active' : ''} onClick={() => switchInputMode('voice')}><Mic size={14} />Voice</button>
        <button type="button" role="tab" aria-selected={inputMode === 'text'} className={inputMode === 'text' ? 'active' : ''} onClick={() => switchInputMode('text')}><Keyboard size={14} />Type</button>
      </div>
      {inputMode === 'text' ? <form className="text-composer" onSubmit={(event) => { event.preventDefault(); void sendTypedMessage(); }}>
        <input value={typedMessage} onChange={(event) => setTypedMessage(event.target.value)} placeholder="Write to ELP…" aria-label="Write a message to ELP" autoComplete="off" disabled={isSendingText} />
        <button className={`text-composer-submit ${isSendingText ? 'is-busy' : ''}`} type="submit" aria-label="Send message" disabled={!typedMessage.trim() || isSendingText}>{isSendingText ? <LoaderCircle size={17} /> : <Send size={17} />}</button>
      </form> : <p className="voice-mode-hint">Tap the ELP core to start or stop voice</p>}
    </section>

    <p className="system-signature">ELP GPT</p>
    {profileOpen && <div className="profile-backdrop" role="presentation" onClick={() => setProfileOpen(false)}><aside className="profile-panel" role="dialog" aria-modal="true" aria-label="Profile and system" onClick={(event) => event.stopPropagation()}><header className="profile-header"><div><p>ELP GPT</p><h2>Profile & System</h2></div><button className="close-button" onClick={() => setProfileOpen(false)} aria-label="Close profile"><X size={21} /></button></header><section className="profile-identity"><div className="profile-avatar"><CircleUserRound size={30} /></div><div><strong>ELP environment</strong><span>Secure voice intelligence profile</span></div><div className="profile-online"><i /> {voice.isActive ? 'Live' : 'Ready'}</div></section><nav className="system-menu" aria-label="ELP system tools">{systemItems.map((item) => <button key={item.key} className={`system-row ${activeSection === item.key ? 'active' : ''}`} onClick={() => openSection(item.key)}><span className="system-row-icon"><SystemIcon kind={item.icon} /></span><span className="system-row-copy"><strong>{item.label}</strong><small>{item.detail}</small></span><ChevronRight size={18} /></button>)}</nav>{renderSection()}<section className="engine-card"><div className="engine-title"><Radio size={17} /><span>Core engines</span></div><div className="engine-grid"><span>Deepgram <StateDot online={Boolean(status?.voice)} /></span><span>Composio <StateDot online={Boolean(status?.composio)} /></span><span>Hermes <StateDot online={Boolean(status?.hermes)} /></span><span>Together AI <StateDot online={Boolean(status?.together)} /></span><span>Honcho <StateDot online={Boolean(status?.memory)} /></span></div></section>{latestVoice && <section className="recent-voice"><span>{latestVoice.role === 'user' ? 'LAST INPUT' : 'LAST RESPONSE'}</span><p>{latestVoice.content}</p></section>}<p className="profile-footnote">Use Voice or Type. Say “ELP, open memory”, “show system status”, or ask ELP to use a connected app.</p></aside></div>}
  </main>;
}
