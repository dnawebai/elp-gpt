...
async function ownerContext(request: Request) {
  ...
  if (!context || context.principal.role !== 'owner') return null;
  return context;
}

export async function GET(request: Request) {
  const context = await ownerContext(request);
  if (!context) return NextResponse.json({ error: 'Authenticated owner authority is required.' }, { status: 401 });
  try {
    const snapshot = await getNotificationDeliverySnapshot(context.profileId);
...
export async function POST(request: Request) {
  ...
  try {
    if (action === 'preferences') {
      ...
      return NextResponse.json({ ok: true, preferences: await setDeliveryPreferences(context.profileId, preferences) }, { headers: { 'Cache-Control': 'no-store, private' } });
    }
    if (action === 'subscribe_push') {
      ...
      await savePushSubscription(context.profileId, {
    ...
    if (action === 'unsubscribe_push') {
      ...
      await removePushSubscription(context.profileId, endpoint);
      return NextResponse.json({ ok: true });
    }
    if (action === 'deliver_now') {
      return NextResponse.json(await deliverPriorityNotifications(context.profileId), { headers: { 'Cache-Control': 'no-store, private' } });
    }
