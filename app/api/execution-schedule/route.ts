...
export async function POST(request: Request) {
  ...
  try {
    if (action === 'refresh') return NextResponse.json({ ok: true, schedule: await runExecutionScheduler({ profileId: profile.profileId, persist: true }) });
    if (action === 'update-policy') {
      ...
      return NextResponse.json({ ok: true, policy, schedule: await runExecutionScheduler({ profileId: profile.profileId, persist: true }) });
    }
