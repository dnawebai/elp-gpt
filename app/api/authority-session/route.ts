...
export async function POST(request: Request) {
  ...
  if (action === 'exchange-access') {
    ...
    if (!consumed) {
      await recordSecurityEventSafe(profile.profileId, {
        category: 'session', action: 'session.access_replay_denied', outcome: 'denied', severity: 'high',
        actorPrincipalId: principal.id, subjectId: principal.id, clientFingerprint: fingerprint,
      });
      return NextResponse.json({ error: 'Principal access grant has already been used.' }, { status: 409 });
    }
    ...
    await recordSecurityEventSafe(profile.profileId, {
      category: 'session', action: 'session.created', outcome: 'success', severity: 'normal',
      actorPrincipalId: principal.id, subjectId: principal.id, sessionId: session.id, clientFingerprint: fingerprint,
    });
  ...
  if (action === 'step-up') {
    ...
    const access = verifyPrincipalAccessToken(typeof body?.accessToken === 'string' ? body.accessToken : undefined);
    if (!access || access.profileId !== context.profileId || access.principalId !== context.principal.id) {
      await recordSecurityEventSafe(context.profileId, {
        category: 'session', action: 'session.step_up_denied', outcome: 'denied', severity: 'high',
        actorPrincipalId: context.principal.id, sessionId: context.session.id, detail: purpose, clientFingerprint: fingerprint,
      });
      return NextResponse.json({ error: 'Fresh access grant for this principal is required.' }, { status: 401 });
    }
    ...
    await recordSecurityEventSafe(context.profileId, {
      category: 'session', action: 'session.step_up_verified', outcome: 'success', severity: 'normal',
      actorPrincipalId: context.principal.id, sessionId: current.id, detail: purpose, clientFingerprint: fingerprint,
    });
  ...
  if (action === 'revoke-current') {
    ...
    await recordSecurityEventSafe(context.profileId, {
      category: 'session', action: 'session.self_revoked', outcome: 'success', severity: 'high',
      actorPrincipalId: context.principal.id, subjectId: context.principal.id, sessionId: context.session.id, clientFingerprint: fingerprint,
    });
  ...
  if (action === 'revoke-session') {
    ...
    const revoked = await revokePrincipalSession(context.profileId, sessionId, context.principal.id);
    await recordSecurityEventSafe(context.profileId, {
      category: 'session', action: 'session.revoked', outcome: 'success', severity: 'high',
      actorPrincipalId: context.principal.id, subjectId: revoked.principalId, sessionId: revoked.id, clientFingerprint: fingerprint,
    });
