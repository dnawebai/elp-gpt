...
function setAuthorityCookie(response: NextResponse, token: string, maxAge = 60 * 60 * 12) {
  response.cookies.set(AUTHORITY_COOKIE, token, {
    httpOnly: true,
    secure: true,
    sameSite: 'lax',
    path: '/',
...
export async function GET(request: Request) {
  ...
  return NextResponse.json({ principal: context.principal, delegated: context.delegated, currentSessionId: context.session?.id || null, sessions }, { headers: { 'Cache-Control': 'no-store, private' } });
}
